...
import Dashboard from '../Dashboard/page'; // Assuming this is needed
...
const Sidebar = () => {
  ...
  return (
    <>
      <aside className={`bg-[#F0F2FE] p-3 transition-all duration-300 min-h-screen ${isCollapsed ? 'w-20' : 'w-80'}`}>
        <nav className="flex w-full flex-col space-y-4 pt-10">
          <div className="flex items-center">
            <Image src="/assets/logo.svg" alt="logo" width={40} height={50} />
            ...
            <button onClick={toggleSidebar} className="relative pl-5 z-50 -mr-16">
              <Image src="/assets/leftpannel.svg" alt="toggle sidebar" width={50} height={50} />
            </button>
          </div>
          ...
          <div className="flex flex-col h-[204px] my-5 gap-5 pl-2 text-[30px] font-Montserrat font-normal">
            <Link href="/">
              <div
                className={`flex items-center pl-3 font-normal border-b-black text-base ${selectedSection === 'Dashboard' ? 'bg-[#08138C] text-white p-3 rounded-2xl' : 'text-[#7C82C5]'}`}
                onClick={() => handleSectionClick('Dashboard')}
              >
                <Image src={selectedSection === 'Dashboard' ? "/assets/dashboardwhite.svg" : "/assets/dashboard.svg"} alt="dashboard" width={30} height={30} />
